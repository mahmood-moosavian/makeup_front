<template>
  <div class="container mx-auto md:w-1/2">
    ...
    <div
      class="sm:max-w-sm flex flex-row items-center justify-between mx-auto mt-24 text-cyan_darkeset_200 font-bold"
    >
    ...
    <div
      class="sm:max-w-sm flex flex-col sm:border mx-auto mt-6 p-5 bg-pink-100 rounded-lg"
    >
      <div class="flex">
      ...
      <div class="flex">
        <input
          maxlength="11"
          :class="{ 'border-red-700': errors.mobile }"
          class="h-10 w-full border text-cyan_darkeset_200 rounded-sm px-3 focus:outline-none my-4"
          type="text"
          name=""
          v-model="mobile"
        />
      ...
      <div class="flex flex-row justify-between mb-2">
        <button
          @click="cancel()"
          class="text-cyan_darker text-md focus:outline-none"
        >
          انصراف
        </button>
        <button
          @click="authenticate()"
          class="text-white py-2 px-2 rounded-full w-24 bg-pink-600 focus:outline-none"
        >
      ...
      <div v-if="errors.mobile" class="text-sm text-right">
        <div class="text-red-600">* {{ errors.mobile[0] }}</div>
      </div>
    </div>
  </div>
</template>
